 ...
 *
 * Commands sent while the device is offline are queued in Durable Object
 * storage (see queueCommand/flushQueuedCommands) and delivered as soon as
 * the device reconnects, rather than being dropped.
 */
export class DeviceRoom {
  ...
  async webSocketMessage(ws, messageRaw) {
    ...
    if (attachment.role === "webapp" && msg.type === "command") {
      await this.forwardCommandToDevice(msg);
      return;
    }
  ...
  async handleRegister(ws, msg, attachment) {
    ...
    if (as === "device") {
      ...
      await this.flushQueuedCommands(ws);
      return;
    }
  ...
  async forwardCommandToDevice(msg) {
    ...
    if (!deviceSocket) {
      // No device connected right now - persist the command so it's
      // delivered as soon as the device reconnects, instead of just
      // failing. Durable Object storage survives hibernation/eviction.
      await this.queueCommand(msg);
      this.forwardToWebapps({
        type: "ack",
  ...
  /**
   * Queues a command for later delivery. For actions where only the
   * latest value matters (set_dnd_level, sync_schedule), a newly queued
   * command replaces any earlier queued command of the same action so
   * the device only ever gets caught up to the most recent desired
   * state, not a backlog of superseded toggles.
   */
  async queueCommand(msg) {
    const LATEST_WINS_ACTIONS = new Set(["set_dnd_level", "sync_schedule"]);
    let queue = (await this.ctx.storage.get("commandQueue")) || [];

    if (LATEST_WINS_ACTIONS.has(msg.action)) {
      queue = queue.filter((q) => q.action !== msg.action);
    }
    ...
    // Cap the queue so a long-offline device doesn't accumulate forever.
    const MAX_QUEUE = 20;
    if (queue.length > MAX_QUEUE) {
      queue = queue.slice(queue.length - MAX_QUEUE);
    }

    await this.ctx.storage.put("commandQueue", queue);
  }

  async flushQueuedCommands(deviceSocket) {
    const queue = (await this.ctx.storage.get("commandQueue")) || [];
    ...
    await this.ctx.storage.delete("commandQueue");
  }
